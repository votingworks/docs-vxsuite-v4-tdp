...
* **Interpreted Ballot Layouts -** Metadata for the position of ballot features such as contests, contest options, and bubbles in the ballot image. The interpreted layout data is used to properly crop and highlight ballot images for adjudication. Included for bubble ballots but not summary ballots.
...
VxSuite does not use any data extensions beyond the NIST specification, but some fields are made required that are not required in the original NIST specification. The two specifications can be compared by comparing the [NIST JSON schema](https://github.com/usnistgov/CastVoteRecords/blob/master/NIST_V0_cast_vote_records.json) with the [VxSuite JSON schema](https://github.com/votingworks/vxsuite/blob/v4.1.0/libs/types/src/cdf/cast-vote-records/vx-schema.json). The additionally required fields are listed in a table below.
...
For bubble ballots, there are both "original" and "interpreted" snapshots. The "original" snapshot contains mark thresholds for every single bubble on the ballot. The "interpreted" snapshot contains only the marks that were detected as definite marks and counted as valid or invalid votes.&#x20;
